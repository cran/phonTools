...
snip = function (object, show = TRUE){
  ...
  if (class (object) == 'sound'){
    ...
    edges = identify(rep(times, length(amps)), rep(amps, each = length(times)), "", n = 2)
    edges = edges %% 1000
    ...
    if (show == TRUE) plot (newtime[seq(1,length(snipped), 2)], snipped[seq(1,length(snipped), 2)], xlab = "Time (ms)", ylab = "Amplitude", type= "l", xaxs = "i")
    invisible (newsound)
  }
  if (class (object) == 'spectrogram'){
    plot (object, pause = FALSE)
    ...
    edges = identify(rep(times, length(freqs)), rep(freqs, each = length(times)), "", n = 2)
    ...
    if (show == TRUE) plot (object, pause = FALSE)
    invisible (object)
  }
}
