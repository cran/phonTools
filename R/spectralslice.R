...
spectralslice = function (sound, padding = length(sound) * 2, fs = 1,  
    show = TRUE, add = FALSE, window = "kaiser", windowparameter = 3, 
    zeromax = TRUE, preemphasisf = 0, type, line = FALSE, removeDC = TRUE, ...){
  ...
  if (add == FALSE & show == TRUE & line == FALSE) 
    plot(hz, power, ylab = "Power (dB.)", xlab = xlab, type = type, xaxs = "i", ...)
  
  if (add == TRUE & show == TRUE & line == FALSE) lines(hz, power, type = type, ...)
