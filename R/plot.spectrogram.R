...
plot.spectrogram <-function (x, y, ylim, quality = FALSE, ...){
  if (x$colors[1] == TRUE)
  ...
  if (!quality){
    image (as.double(times), as.double(hz), x$spectrogram, useRaster = FALSE, col = zcolors, xlab = 'Time (ms)', ylab = "Frequency (Hz)")
  }
}
