Ffilter = function (sound, ffs, bwp = 0.06, fs = 22050, verify = FALSE){
  ...
  nffs = length (ffs[[1]])
  if (length(ffs) != 2) stop ('Only initial and final formant values may be provided.')
  if (length(ffs[[1]]) != length(ffs[[2]])) stop ('Same number of formants must be provided for beginning and
                                                    end points.')
  ...
  if (length(bwp) != nffs) stop ('Number of bandwidths specificed must equal number of formants.')
  ...
  for (j in nffs:1) {
    CF = ffs[,j]
    if (percent) BW = (CF * bwp[j])
    if (!percent) BW = rep (bwp[j], length(sound))
    ...
    for (i in 3:length(old)) new[i] = old[i]*A[i] + new[i - 1]*B[i]*sqrt(A[i]/A[i-1]) + new[i - 2]*C[i]*sqrt(A[i]/A[i-1])
    #for (i in 3:length(old)) new[i] = old[i]*A[i] + new[i - 1]*B[i] + new[i - 2]*C[i]
    old = new
  }
  if (verify == TRUE) {
    par(mfrow = c(2, 1))
